...
int main(int argc, char **argv)
{
    ...
    ds = NULL;
    if (enc)
        cs = bv32_encode_alloc();
    else
        ds = bv32_decode_alloc();
    ...
    for (;;)
    {
        ...
        if (enc == 1)
        {
            ...
            bv32_encode(cs, PackedStream, x);
            itu_codec_bitstream_write(PackedStream, 160, packing, fo);
        }
        else
        {
            ...
            if (bfi)
                ...
                bv32_decode(ds, x, PackedStream);
            fwrite(x, sizeof(int16_t), BV32_FRAME_LEN, fo);
        }
    }

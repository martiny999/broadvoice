...
int main(int argc, char **argv)
{
    ...
    ds = NULL;
    if (enc)
        cs = bv16_encode_alloc();
    else
        ds = bv16_decode_alloc();
    ...
    for (;;)
    {
        ...
        if (enc == 1)
        {
            ...
            bv16_encode(cs, PackedStream, x);
            itu_codec_bitstream_write(PackedStream, 80, packing, fo);
        }
        else
        {
            ...
            if (bfi)
                ...
                bv16_decode(ds, x, PackedStream);
            fwrite(x, sizeof(int16_t), BV16_FRAME_LEN, fo);
        }
    }
